    ...
    data.forEach(item => {
        ...
        if(item[key] && typeof item[key] === 'string') {
            ...
            values.forEach(value => {
                if (value) { // Check if the value is not an empty string after trimming
...
function convertAllAgesToYears(age, unit) {
    switch (unit) {
        case 'y': return age;           // Age is already in years
        case 'm': return age / 12;     // Convert months to years
        case 'h': return age / (365*24); // Convert hours to years
        default: return age;
    }
}
