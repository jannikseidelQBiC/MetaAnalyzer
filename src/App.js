...
function App() {
  ...
  return (
    <div className="App">
      <center><h1>DZIF Metadata Analyzer</h1></center>
      {!showAnalyzer
       ? <ExcelReader onDataUploaded={({data, fileName}) => {setExcelData(data); setFileName(fileName)}} />
       : null
      }
      {!showAnalyzer
      ? null
      ...
      {analysisCompleted && <div>
        ...
        <div>
          <center>
          <RestartButton /> <ExcelExporter data={excelData} result={analyzedData} fileName={fileName}/>
          </center>
        </div>
        <br></br>
        <div>
